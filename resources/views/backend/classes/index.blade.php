...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">{{__("classe.title")}}</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('classes.create') }}" class="bg-gray-200 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
                    ...
                    <span class="ml-2 text-xs font-semibold">{{__("classe.add")}}</span>
                </a>
            </div>
        </div>
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                ...
                <div class="w-2/12 px-4 py-3">{{__("classe.table_name")}}</div>
                <div class="w-1/12 px-4 py-3">{{__("classe.table_student")}}</div>
                <div class="w-4/12 px-4 py-3 text-center">{{__("classe.table_subject")}}</div>
                <div class="w-2/12 px-4 py-3">{{__("classe.table_teatcher")}}</div>
                <div class="w-2/12 px-4 py-3 text-right">{{__("classe.table_action")}}</div>
            </div>
